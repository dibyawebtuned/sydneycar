...
const SloganBanner: React.FC = () => {
  ...
  const Button: React.FC<ButtonProps> = ({ number, name }) => (
    <button className="relative overflow-hidden bg-linear-to-r from-[#FEC130] to-[#FFC85C] px-5 py-2 flex items-center gap-2 text-[#044E9E] cursor-pointer group justify-center shadow-md hover:shadow-lg transition-shadow duration-300 w-full md:w-auto">
      <Phone className="w-4 h-4 fill-[#044E9E] group-hover:translate-x-1 transition-transform duration-300" />
      <div
        className={`flex flex-row items-center gap-1 text-[14px] font-semibold ${inter.className}`}
      >
      ...
      <span className="absolute bottom-0 left-0 w-0 h-0.5 bg-w[#022F6C] group-hover:w-full transition-all duration-300"></span>
    </button>
  );
